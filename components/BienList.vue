...
const emit = defineEmits(['selectBien', 'update:layoutView', 'update:selectedBien'])

const { layoutView, selectedBien } = useVModels(props, emit)
...
const [valueViewMap, toggleViewMap] = useToggle()
...
<template>
  <div :class="[valueViewMap ? 'translate-x-[calc(100%-70px)] shadow-md' : 'translate-x-0']" class="relative top-0 z-5 h-[calc(100vh-6.175rem)] min-h-160 w-full flex flex-1 flex-row-reverse transition-all duration-300 dark:text-light">
    <template v-if="valueViewMap">
      <div class="absolute right-0 top-0 z-21 h-full w-full bg-light/70 transition-all duration-400 dark:bg-black/70" @click="(selectedBien = null, toggleViewMap())" />

      <div class="absolute left--3rem top-0">
        <a-tooltip content="Toggle Map" position="br" mini>
          <a-button class="px-2 !h-10 !w-12 !rounded-0 !border-none !bg-light-3 !dark:bg-black" block @click="(selectedBien = null, toggleViewMap())">
            <template #icon>
    ...
    <div
      ...
      :class="[layoutView !== 'MAP' ? 'w-2/2' : 'md:w-4/6 lg:w-1/2']"
    >
      <div
              ...
              <a-trigger trigger="click" show-arrow>
                ...
                <template #content>
                  <div class="demo-arrow w-80 p-2">
                    <div class="m--2 bg-gray/10 p-1 text-3.5/7 font-sans">
                      Budget
                    </div>
                    <div class="pt-4">
                      <a-slider
                ...
                <div v-if="!errorBiens" ref="contentDataRef" class="m-0 h-full">
                  ...
                  >
                    <template #actions="{ record }">
                      <div class="flex space-x-1">
                        <a-button size="mini" @click="(selectedBien = record, toggleViewMap())">
                          view in map
                        </a-button>
